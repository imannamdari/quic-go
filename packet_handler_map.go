	...
	if err := setReceiveBuffer(c, logger); err != nil {
		if !strings.Contains(err.Error(), "use of closed network connection") {
			receiveBufferWarningOnce.Do(func() {
				...
				log.Printf("%s. See https://github.com/imannamdari/quic-go/wiki/UDP-Receive-Buffer-Size for details.", err)
			})
		}
	}
	conn, err := wrapConn(c)
...
func (h *packetHandlerMap) listen() {
	...
	for {
		...
		// See https://github.com/imannamdari/quic-go/issues/1737 for details.
